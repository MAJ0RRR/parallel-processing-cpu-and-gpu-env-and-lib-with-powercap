...
from charts import time_powercap_scatter, time_batch_size_scatter, time_number_of_nodes_bar, \
    time_number_of_nodes_scatter, total_energy_used_powercap
        ...
        experiment_configurations=list(
            chain.from_iterable(
                [
                    ...
                    for powercap in range(100, 3001, 100)
                ]
            )
        )
    )
...
def experiment_total_energy_used_powercap(description: str, app_name: str, file_path: str | os.PathLike):
    common_run_parameters = functools.partial(
        RunParameters,
        app_name=app_name,
        batch_size=51200,
